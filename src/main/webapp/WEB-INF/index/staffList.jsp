          ...
          <div class="pagebody11_top"><!--输入用户名-->
          	<form id="searchStaffForm" name="searchStaffForm" action="${ctx}/staff/staffList" method="post">
               ...
               <ul class="page">
					    ...
					    <a href="${ctx}/staff/staffList?pageNum=${pageNo}&pageSize=10&username=${username}" style="background: #C30D23 none repeat scroll 0% 0%;color:#ffffff;">${pageNo}</a>
					     </c:when>
					   <c:otherwise>
					   <a href="${ctx}/staff/staffList?pageNum=${pageNo}&pageSize=10&username=${username}" >${pageNo}</a>
					   </c:otherwise>
					   </c:choose>
